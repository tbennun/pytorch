...
class CAFFE2_API Tensor {
 ...
 public:
  ...
  /// The returned Tensor is a view of the current Tensor without
  /// any dual at the selected level level.
  Tensor fw_primal(uint64_t level) const {
    if (!this->defined()) {
      return Tensor();
    } else {
      auto result = this->tensor_data();
